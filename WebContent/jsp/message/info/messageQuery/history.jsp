	...
	<nav class="navbar navbar-default">
		<div class="container-fluid">
			<a type="button" href="welcome.jsp"
				class="btn btn-primary navbar-btn">主页</a>
			...
			<table class="table table-bordered">
				<tbody>
					<tr>
						...
						<th width="5">收件人</th>
						<th width="10">发送时间</th>
						<th width="40">被发件人删除</th>
						<th width="45">被发件人永久删除</th>
						<th width="40">被收件人查看</th>
						<th width="40">被收件人删除</th>
						<th width="45">被收件人永久删除</th>
					</tr>
					...
					<c:if test="${result.found}">
						<c:forEach var="r" items="${result.rows}" varStatus="vs">
							<tr ${r.flag==1?"locked":""}>
								<td>${vs.count}.</td>
								<td><a href="sysuser/check.do?id=${r.id}">查看/修改/删除</a></td>
								<td>${r.account}</td>
								<td>${r.name}</td>
								<td>${r.phone}</td>
								<td><c:choose>
