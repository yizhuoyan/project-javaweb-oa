...
@WebServlet("/email/ListSentEmailServlet.do")
public class ListSentEmailServlet extends CommonServlet {

	private static final long serialVersionUID = -1670046073710146013L;
	...
	protected String handleRequest(HttpServletRequest req, HttpServletResponse resp) throws Throwable {
		...
		try {
			...
			PaginationQueryResult<EmailEntity> result = fun.querySentEmailByKey(uc.getName(), key, pageNo, pageSize);
			req.setAttribute("result", result);
			// 3确定视图
		} catch (OAException e) {
			req.setAttribute("message", e.getMessage());
		}
		return "/jsp/message/email/outbox/outbox.jsp";
	}
}
